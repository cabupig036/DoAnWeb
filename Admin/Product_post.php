<?php 
	...
	include "config.php";
	if ($action == 'create' && $image && $product_name && $gia) {
		$name       = str_replace(' ', '-', $image["name"]);
        $bannerpath ="images/Product/" . $name;
        $flag       = move_uploaded_file($image["tmp_name"],$bannerpath);
        $sql        = "INSERT INTO `product`(`product_name`, `image`, `gia`) VALUES ('{$product_name}', '{$name}', '{$gia}')";
	...
	if ($action == 'edit' && $product_name && $id && $gia) {
		if ($image) {
			$name       = str_replace(' ', '-', $image["name"]);
            $bannerpath ="images/Product/".$name;
            $flag       = move_uploaded_file($image["tmp_name"],$bannerpath);
            $sql        = "UPDATE `product` SET `id`='{$id}',`product_name`='{$product_name}',`image`='{$name}',`gia`='{$gia}' WHERE `id`='{$id}'";
		} else {
			$sql        = "UPDATE `product` SET `product_name`='{$product_name}',`gia`='{$gia}' WHERE `id`='{$id}'";
		}
